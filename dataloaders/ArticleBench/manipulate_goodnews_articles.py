...
from dataloaders.dataloader_original_articles import Goodnews_article
...
def spacy_split_sentence(input_article):
...
def sample_from_model(texts, base_model, base_tokenizer,
    ...
    # original_sentences = spacy_split_sentence(texts[0])
    ...
    # generated_sentences = spacy_split_sentence(generated_text)
    try:
...
if __name__=="__main__":
    ...
    parser.add_argument('--file_path', type=str, default="data/Goodnews/goodnews_test.jsonl",
    ...
    parser.add_argument('--output_dir', type=str, default="data/Text_Localization/Goodnews")
    parser.add_argument('--article_num', type=int, default=11,) # set number of article to 5 for DEBUG purpose
    ...
    if "goodnews" in args.file_path:
        news_data = Goodnews_article(file_path=args.file_path, article_num=args.article_num)
    ...
    batch_id = 0
    all_samples = []
    for batch in tqdm(news_dataloader, desc="manipulate Goodnews articles"):
        # if batch_id<10: # special_key="5515ba1e38f0d84d44f0ddb0"
        #     batch_id+=1;continue
